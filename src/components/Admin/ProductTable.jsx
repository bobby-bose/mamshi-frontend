...
const ProductTable = () => {
            ...
            renderCell: (params) => {
                return (
                    <>
                        {
                            params.row.stock < 10 ? (
                                <span className="font-medium text-red-700 rounded-full bg-darkGray-200 p-1 w-6 h-6 flex items-center justify-center">{params.row.stock}</span>
                            ) : (
    ...
    return (
        <>
            <MetaData title="Admin Products | Slouch" />

            {loading && <BackdropLoader />}

            <div className="flex justify-between items-center">
                <h1 className="text-lg font-medium uppercase">products</h1>
                <Link to="/admin/new_product" className="py-2 px-4 rounded shadow font-medium text-white bg-gray-700 hover:shadow-lg">New Product</Link>
            </div>
