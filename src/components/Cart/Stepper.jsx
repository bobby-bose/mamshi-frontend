...
const Stepper = ({ activeStep, children }) => {
    ...
    const steps = [
        {
            label: "LOGIN",
            desc: <p className="font-medium text-sm">{user} <span className="text-sm font-normal">{user.email}</span></p>
        },
        {
            label: "DELIVERY ADDRESS",
            desc: <p className="font-medium text-sm">{user} <span className="text-sm font-normal">{address}</span></p>
        },
        {
            label: "ORDER SUMMARY",
                    ...
                    <>
                        {activeStep === index ? (
                            <div className="flex flex-col shadow rounded-sm">
                                <div className="flex items-center rounded-t-sm bg-gray-700 px-6 py-2 gap-4">
    ...
    return (
        <div className="flex bg-white shadow px-4 py-3 pb-4 rounded-sm">
            ...
            <div className="flex flex-col mt-1 gap-0.5">
                <h2 className="font-medium text-stone flex items-center gap-2">{label}
                    {isDesc && (
