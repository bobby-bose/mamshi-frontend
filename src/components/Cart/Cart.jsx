...
// check if there is a productName and productDescription in sessionStorage or else redirect to login page
  ...
  useEffect(() => {
    axios.get(`http://localhost:4000/api/v1/wishlist`)
      .then(async (response) => {
        ...
        // Fetch product + user details only for this user’s items
          ...
          >
            ...
            <div className="mt-4 sm:mt-0 sm:ml-4 flex-1">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {["S", "M", "L", "XL"].map(
                  (size) =>
                    item.product?.productDetails?.[size] && (
                      <span
                        key={size}
                        className="border px-2 py-1 text-sm rounded-md bg-gray-100"
                      >
                        {size}
                      </span>
                    )
